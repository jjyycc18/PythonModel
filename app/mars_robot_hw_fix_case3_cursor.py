...
def mars_time_process(step_seq, eqp_id, lot_id, wafer_id, time_var):
    try:     
        ...
        # 12. 결과 있으면 time_var 설정대로 결과 생성
        result_list = []
        if not filtered_process_hist_df.empty:
            grouped = filtered_process_hist_df.groupby('module_id')
            if time_var == 'START_TIME':
                # module_id별 최소 starttime_rev
                result_list = [row for row in grouped['starttime_rev'].min()]
            elif time_var == 'END_TIME':
                # module_id별 최대 endtime_rev
                result_list = [row for row in grouped['endtime_rev'].max()]
            elif time_var == 'PROCESS_TIME':
                # module_id별 (최대 endtime_rev - 최소 starttime_rev).total_seconds()
                for module_id, group in grouped:
                    min_start = group['starttime_rev'].min()
                    max_end = group['endtime_rev'].max()
                    # pandas.Timestamp -> datetime 변환 후 차이 계산
                    if pd.notnull(min_start) and pd.notnull(max_end):
                        duration = (pd.to_datetime(max_end) - pd.to_datetime(min_start)).total_seconds()
...
if __name__ == "__main__":
    ...
    src_var = 'LOADLOCK'
    dst_var = 'VTM'
    robot_time_var = 'PROCESS_TIME'

    work_var = 'LOADPORT'
    state_var = 'FOUPDOOROPEN'
    hw_time_var = 'PROCESS_TIME'

    start_time = time.time()
    
    # Test mars_time_robot
    robot_result = mars_time_robot(step_seq, eqp_id, lot_id, wafer_id, src_var, dst_var, robot_time_var)
    logger.info(f"Robot result: {robot_result}")

    # Test mars_time_hw
    hw_result = mars_time_hw(step_seq, eqp_id, lot_id, wafer_id, work_var, state_var, hw_time_var)
    ...
    process_result = mars_time_process(step_seq, eqp_id, lot_id, wafer_id, hw_time_var)
    logger.info(f"HW process result: {process_result}") 
