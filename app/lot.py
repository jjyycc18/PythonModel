...
def predict_ultimate_analysis(handler, all_data, latest_round):
    ...
    #jyc
    excluded_nums.extend( [final_recommendation[i] for i in [0,1,3,5]] )
...
def generate_final_7_sets(handler, final_prediction, all_data, latest_round, earliest_round):
    ...
    #jyc
    final_in.extend(available_recent_numbers)
...
def generate_rule_based_sets(handler, excluded_nums, remain_nums, all_data, latest_round):
    ...
    for rank, (set_num, score, final_set) in enumerate(set_scores, 1):
        ...
        #jyc 
        setss.extend(set(final_set))

    #jyc
    print(f" ** 추  천 : {final_in}")
    print(f" ** 안 추천 : {set(setss)}")
    intersacts = list(set(final_in) & set(setss))
    intersacts.sort()
    print(f" ** 교집함 : {set(intersacts)}")
    sorted_final_in = list(set(final_in) - set(setss))
    sorted_final_in.sort()
    print(f" ** {next_ch}회 추천 : {sorted_final_in}")
    unions = list(set(sorted_final_in) | set(intersacts))
    unions.sort()
    print(f" ** {next_ch}회 교집합 추천 : {unions}")
    nums = list(range(1,46))
    remain_nums = list(set(nums) - set(unions))
    remain_nums.sort()
    print(f" ** {next_ch}회 잔여수 : {remain_nums}")
    #jyc
